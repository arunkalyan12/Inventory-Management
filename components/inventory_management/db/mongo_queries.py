...
def predict_image(
    ...
    for result in results:
        for box in result.boxes:
            ...
            logger.info(
                f"Detected {label} with confidence {conf:.2f} at bbox {x1, y1, x2, y2}"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="YOLOv8 prediction script with shared logger"
    )
    ...
    preds = predict_image(args.image, conf_threshold=args.conf, save_result=args.save)
